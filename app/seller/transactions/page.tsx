  ...
  useEffect(() => {
    ...
    async function fetchAll() {
      ...
      try {
        ...
        if (isMounted) {
          setTransactions(data.filter((t) => t.status !== TRANSACTION_STATUS.PENDING_SELLER_CONFIRM));
        }
      } catch {
              ...
              {STATUS_TABS.map((tab) => {
                ...
                return (
                  ...
                  >
                    {tab.label}
                    {count > 0 && (
                      <span className={`text-xs font-bold rounded-full px-1.5 py-0.5 min-w-5 text-center ${isActive ? "bg-orange-500 text-white" : "bg-gray-100 text-gray-500"
                        }`}>
                        {count}
                      </span>
                    )}
                  ...
                  <tbody className="divide-y divide-gray-50">
                    {filteredTransactions.map((t) => (
                      ...
                      >
                        ...
                        {/* Loại */}
                        <td className="px-6 py-4">
                          <span className={`text-xs font-bold px-2.5 py-1 rounded-lg ${t.transactionType === TRANSACTION_TYPE.PURCHASE
        ...
        {STATUS_TABS.map((tab) => {
          ...
          return (
            <button
              key={tab.key}
              onClick={() => setFilterStatus(tab.key)}
              className={`px-3.5 py-2 rounded-lg text-sm font-semibold transition-all duration-200 whitespace-nowrap flex items-center gap-1.5 ${isActive
            ...
            >
              ...
              {count > 0 && (
                <span
                  className={`text-xs font-bold rounded-full px-1.5 py-0.5 min-w-5 text-center leading-none ${isActive
        ...
        <div className="lg:hidden">
          {filteredTransactions.map((t, idx) => (
            <div
              key={t.transactionId}
              onClick={() => router.push(`/transactions/${t.transactionId}`)}
              className={`px-4 py-4 flex items-start gap-3 cursor-pointer active:bg-orange-50 transition-colors duration-150 border-b border-gray-50 last:border-0 ${idx % 2 === 0 ? "bg-white hover:bg-orange-50/20" : "bg-slate-50/40 hover:bg-orange-50/30"
                }`}
            >
            ...
            <tbody>
              {filteredTransactions.map((t, idx) => (
                <tr
                  key={t.transactionId}
                  onClick={() => router.push(`/transactions/${t.transactionId}`)}
                  className={`border-b border-gray-50 hover:bg-orange-50/30 transition-colors duration-150 group cursor-pointer ${idx % 2 === 0 ? "bg-white" : "bg-slate-50/40"
                    }`}
                >
                  {/* Mã đơn / Xe */}
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4 text-right">
                    <p className="text-sm font-black text-gray-900">{formatPrice(t.totalAmount)}</p>
                    <span className={`mt-0.5 inline-block text-xs px-1.5 py-0.5 rounded-md font-bold ${t.transactionType === TRANSACTION_TYPE.PURCHASE
                        ? "bg-blue-50 text-blue-600"
                        : "bg-orange-50 text-orange-600"
                      }`}>
                      {TRANSACTION_TYPE_LABELS[t.transactionType as keyof typeof TRANSACTION_TYPE_LABELS] || t.transactionType}
