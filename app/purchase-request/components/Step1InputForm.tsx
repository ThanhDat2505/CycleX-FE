...
const STYLES = {
  ...
  checkIcon:
    "absolute top-3 right-3 text-brand-primary animate-scale-in bg-white rounded-full p-0.5 shadow-sm",
  disabledOption:
    "mt-6 p-5 border border-gray-100 rounded-2xl bg-gray-50/50 opacity-60 flex items-center justify-between",
  ...
  hintBlue:
    'text-xs text-brand-primary mt-2 pl-1 font-semibold flex items-center gap-1 before:content-[\"●\"] before:text-[8px]',
...
function getMinDateString(): string {
  return new Date(Date.now() + MIN_DAYS_AHEAD * 24 * 60 * 60 * 1000)
    .toISOString()
    .split("T")[0];
}
...
export default function Step1InputForm({
  ...
  const handleChange = useCallback(
    (field: keyof PurchaseRequestForm, value: string | number | undefined) => {
      onFormDataChange({
        ...formData,
        [field]: value,
      });
    ...
    <div className={STYLES.card}>
      ...
      <div className="mb-8">
        <label className="block text-gray-700 font-semibold mb-4">
          {MESSAGES.S50_PAYMENT_METHOD_LABEL}{" "}
            ...
            <div>
              <div className="flex items-center gap-2">
                <span className="font-medium text-gray-500">
                  {MESSAGES.S50_PAYMENT_ONLINE_TITLE}
                </span>
      ...
      <div className={`${STYLES.fieldGrid} mb-8`}>
        ...
        <div
          className={
            formData.transactionType === "DEPOSIT" ? "" : "md:col-span-2"
          }
        >
          <Input
          ...
          />
          {!errors.desiredTime && (
            <p className={STYLES.hint}>{MESSAGES.S50_DATE_MIN_HINT}</p>
          )}
        </div>
      </div>
            ...
            <AddressSelector
              ...
              onSelect={(addr) => {
                ...
                if (addr) {
                  onFormDataChange({
                    ...formData,
                    receiverName: addr.receiverName || formData.receiverName,
                    receiverPhone: addr.receiverPhone || formData.receiverPhone,
                    receiverAddress: addr.fullAddress,
                  });
                }
              }}
            />
            {errors.receiverAddress && !selectedAddress && (
              <p className="text-sm text-red-500">{errors.receiverAddress}</p>
            )}
          </div>
        ) : (
          <>
