...
const STYLES = {
    ...
    lineActive: 'absolute left-0 top-1/2 -translate-y-1/2 h-1 bg-gradient-to-r from-orange-400 via-brand-primary to-brand-primary-hover rounded-full -z-10 transition-all duration-1000 ease-in-out shadow-[0_0_15px_rgba(255,107,0,0.3)]',
    ...
    circle: (isActive: boolean, isCurrent: boolean) => `
        ...
        ${isActive
            ? 'bg-brand-primary border-brand-primary text-white scale-110 shadow-lg shadow-orange-200 rotate-6'
            : 'bg-white border-gray-100 text-gray-300'
        }
        ${isCurrent ? 'animate-pulse-slow shadow-[0_0_20px_rgba(255,107,0,0.5)] !rotate-0 !scale-125' : ''}
    `,
    label: (isActive: boolean, isCurrent: boolean) => `
        mt-3 text-[10px] font-black uppercase tracking-[0.2em] transition-all duration-500
        ${isActive ? 'text-orange-950 opacity-100 translate-y-0' : 'text-gray-300 opacity-50 translate-y-1'}
