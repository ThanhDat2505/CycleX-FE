...
export function NotificationItem({
  ...
  return (
    ...
    >
      <div
        className={`flex-shrink-0 w-10 h-10 rounded-full flex items-center justify-center mr-4 ${getIconBackground(type)}`}
      >
        {getIcon(type)}
      </div>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex justify-between items-start mb-1">
          <p
            className={`text-sm font-medium ${!isRead ? "text-gray-900" : "text-gray-700"} truncate pr-4`}
          >
            {title}
          </p>
        ...
        <p
          className={`text-sm ${!isRead ? "text-gray-600" : "text-gray-500"} line-clamp-2`}
        >
          {message}
        </p>
      </div>
