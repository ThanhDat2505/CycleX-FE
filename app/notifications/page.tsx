...
export default function NotificationsPage() {
  ...
  const handleMarkAllAsRead = async () => {
    ...
    window.dispatchEvent(
      new CustomEvent("notifications-read", { detail: { count: 0 } }),
    );
  ...
  const handleNotificationClick = useCallback(
    async (notif: NotificationResponse) => {
      if (!user?.userId) return;
      ...
      if (!notif.isRead) {
        ...
        // Notify header bell to update immediately
        window.dispatchEvent(
          new CustomEvent("notifications-read", { detail: { delta: 1 } }),
        );
      ...
      try {
        ...
        const isValid = await notificationService.validateRelatedObject(
          notif.type,
          notif.relatedId,
        );
