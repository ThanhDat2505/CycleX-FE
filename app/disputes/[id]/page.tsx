...
import { getDisputeById } from '@/app/services/disputeServices';
    ...
    if (isLoading) {
        return (
            <div className="min-h-screen flex flex-col items-center justify-center p-4 bg-gray-50">
                ...
                <p className="mt-4 text-gray-500 font-bold animate-pulse uppercase tracking-wider text-xs">Đang tải kết quả khiếu nại...</p>
            </div>
        );
    }
        ...
        return (
            <div className="min-h-screen flex items-center justify-center p-4 bg-gray-50">
                <div className="max-w-md w-full bg-white rounded-3xl shadow-xl p-8 text-center animate-scale-in border border-gray-100">
                    ...
                    <h2 className="text-2xl font-black text-gray-900 mb-2 tracking-tight">Lỗi truy cập</h2>
                    <p className="text-gray-500 mb-8 leading-relaxed">{error || 'Khiếu nại này không tồn tại hoặc bạn không có quyền truy cập.'}</p>
                    <div className="flex flex-col gap-3">
                        <Button variant="primary" onClick={() => window.location.reload()} className="w-full py-4 rounded-2xl font-bold shadow-lg shadow-orange-100">
                            Thử lại
                        </Button>
                        <Button variant="outline" onClick={() => router.push('/transactions')} className="w-full py-4 rounded-2xl font-bold border-gray-200 text-gray-500">
                            Quay lại giao dịch
                        </Button>
                    </div>
                </div>
            ...
            <div className="max-w-3xl mx-auto">
                ...
                <div className="bg-white rounded-3xl shadow-xl overflow-hidden mb-8 border border-gray-100">
                    <div className={`p-8 text-center ${
                        isSolved ? 'bg-green-50' : isRejected ? 'bg-red-50' : 'bg-orange-50'
                    }`}>
                        <div className={`w-20 h-20 rounded-2xl flex items-center justify-center mx-auto mb-6 shadow-sm ${
                            isSolved ? 'bg-green-100 text-green-600' : isRejected ? 'bg-red-100 text-red-600' : 'bg-orange-100 text-brand-primary'
                        }`}>
                            {isSolved ? (
                                <svg className="w-10 h-10" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <h1 className={`text-3xl font-black tracking-tight mb-2 ${
                            isSolved ? 'text-green-900' : isRejected ? 'text-red-900' : 'text-orange-900'
                        }`}>
                            {isSolved ? 'Khiếu nại được chấp nhận' : isRejected ? 'Khiếu nại bị từ chối' : 'Đang chờ giải quyết'}
                        </h1>
                        <p className={`text-sm font-bold uppercase tracking-widest opacity-70 ${
                            isSolved ? 'text-green-700' : isRejected ? 'text-red-700' : 'text-orange-700'
                        }`}>
                    ...
                    {(isSolved || isRejected) && dispute.adminNote && (
                        <div className="p-8 bg-white border-t border-gray-100">
                            <div className="flex items-center gap-3 mb-4">
                                ...
                                <h4 className="text-sm font-black text-gray-800 uppercase tracking-widest">Quyết định từ CycleX</h4>
                            </div>
                ...
                <div className="bg-white rounded-3xl shadow-xl overflow-hidden border border-gray-100 p-8 space-y-8">
                    ...
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-6 border-b border-gray-100">
                        <div>
                            <h2 className="text-2xl font-black text-gray-900 tracking-tight">{dispute.title}</h2>
                        ...
                        <div className="flex items-center gap-4">
                            <div className="text-right">
                                <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest">Đơn hàng</p>
                                <p className="text-sm font-black text-gray-900 leading-tight">#{dispute.orderId}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="space-y-4">
                        <div className="flex items-center gap-3">
                            ...
                            <h4 className="text-sm font-black text-gray-800 uppercase tracking-widest">Nội dung chi tiết</h4>
                        </div>
                ...
                <div className="mt-12 text-center space-y-6">
                    <p className="text-gray-400 text-[11px] font-bold uppercase tracking-[0.2em] max-w-sm mx-auto leading-loose">
                        CycleX đảm bảo tính minh bạch và công bằng cho mọi giao dịch. Quyết định của Ban quản trị là quyết định cuối cùng.
                    </p>
                    <div className="flex justify-center gap-4">
                        <Button variant="outline" onClick={() => router.push('/')} className="rounded-2xl px-8 border-gray-200 text-gray-500 hover:bg-white hover:shadow-md transition-all">
                            Về Trang Chủ
                        </Button>
                        <Button variant="primary" onClick={() => router.push('/transactions')} className="rounded-2xl px-8 shadow-lg shadow-orange-100">
