...
export default function DisputeResultPage() {
  ...
  const [disputeResult, setDisputeResult] =
    useState<DisputeResultResponse | null>(null);
  ...
  const handleReply = async (e: React.FormEvent) => {
    ...
    if (!replyContent.trim() && selectedImages.length === 0) {
      addToast(
        "Vui lòng nhập nội dung hoặc đính kèm ít nhất 1 hình ảnh.",
        "error",
      );
      return;
    }
    ...
    try {
      ...
      const updated = await replyToDispute(
        disputeId,
        replyContent.trim() || "(Đính kèm hình ảnh)",
        uploadedUrls,
      );
      setDispute(updated);
      setReplyContent("");
  ...
  const handleOverrideSubmit = async (e: React.FormEvent) => {
    ...
    if (!overrideReason.trim()) {
      addToast("Vui lòng nhập lý do ghi đè kết quả.", "error");
      return;
    }
    ...
    try {
      ...
      addToast(
        "Đã ghi đè kết quả thành công! Thông báo đã gửi tới Buyer & Seller.",
        "success",
      );
      ...
      addToast(err.message || "Lỗi khi ghi đè kết quả khiếu nại.", "error");
    } finally {
  ...
  const statusLabel = (() => {
    switch (dispute.status) {
      ...
      case "ESCALATED":
        return "Đã chuyển lên quản trị viên";
      case "RESOLVED":
        return "Khiếu nại được chấp nhận";
      case "REJECTED":
        return "Khiếu nại bị từ chối";
      default:
        return "Đang được xem xét";
    }
  })();
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-6 pb-8 border-b border-gray-200">
          <div className="animate-slide-up">
            ...
            <p className="text-[11px] font-black text-gray-500 tracking-[0.3em] uppercase">
        ...
        <div className="space-y-10">
          ...
          <div className="animate-fade-in">
            ...
            >
              ...
              <div
                className={`absolute -top-24 -left-24 w-48 h-48 blur-[80px] rounded-full opacity-20 ${
                  isSolved
                    ? "bg-emerald-500"
                    : isRejected
                      ? "bg-rose-500"
              ...
              <div
                className={`w-24 h-24 rounded-3xl mx-auto mb-8 flex items-center justify-center border-2 transition-transform hover:scale-110 duration-500 ${
                  isSolved
                  ...
                  <div className="mb-4">
                    ...
                    >
                      {disputeResult.result === "BUYER_WIN"
                        ...
                        : disputeResult.result === "SELLER_WIN"
                          ? "Người bán thắng - Giải phóng thanh toán"
                          : disputeResult.result === "SPLIT"
                            ? "Phân chia 50/50"
                            : disputeResult.result === "CLOSED"
                              ? "Đã đóng"
              ...
              {!disputeResult &&
                ...
                dispute.adminNote && (
                  <div className="mt-10 pt-10 border-t border-gray-200 text-left bg-gray-50 -mx-10 px-10 rounded-b-[2.5rem]">
                    ...
                    <p className="mt-4 text-[9px] font-bold text-gray-600 uppercase tracking-widest">
                      Ghi nhận lúc: {formatDate(dispute.resolvedAt || "")}
                    </p>
                  </div>
                )}
                ...
                <form onSubmit={handleReply}>
                  ...
                  <div className="flex justify-end gap-3 mt-4">
                    <button
                      ...
                      disabled={
                        isReplying ||
                        (!replyContent.trim() && selectedImages.length === 0)
                      }
          ...
          <div className="bg-white border border-gray-200 rounded-[2.5rem] p-10 max-w-lg w-full shadow-2xl animate-scale-in relative overflow-hidden">
            ...
            <div className="text-center mb-8">
              ...
              <p className="text-xs text-purple-400 font-black uppercase tracking-widest">
                Thao tác cấp quyền quản trị cao cấp (BP7)
              </p>
            </div>
            ...
            <form onSubmit={handleOverrideSubmit} className="space-y-6">
              <div>
                <label className="block text-xs font-black text-gray-500 uppercase tracking-[0.2em] mb-3 ml-1">
                  Kết quả bàn giao mới
                </label>
                <div className="grid grid-cols-1 gap-3">
                  {[
                    {
                      id: "BUYER_WIN",
                      label: "Buyer Thắng - Hoàn trả toàn bộ tiền",
                    },
                    {
                      id: "SELLER_WIN",
                      label: "Seller Thắng - Giải phóng thanh toán",
                    },
                    {
                      id: "SPLIT",
                      label: "Phân chia 50/50 - Mỗi bên nhận một nửa",
                    },
                  ].map((opt) => (
                    <button
                      key={opt.id}
              ...
              <div className="grid grid-cols-2 gap-4 pt-4">
                ...
                >
                  {isOverriding ? (
                    <>
                      <Loader2 size={14} className="animate-spin" /> Đang cập
                      nhật...
                    </>
                  ) : (
                    "Xác nhận Override"
