...
const STYLES = {
  card: "group relative bg-white rounded-2xl shadow-sm hover:shadow-2xl transition-all duration-500 cursor-pointer flex flex-col h-full border border-gray-100 hover:-translate-y-2",
  imageWrapper: "relative aspect-[4/3] overflow-hidden bg-gray-100",
  image:
    "w-full h-full object-cover transition-transform duration-700 group-hover:scale-110",
  imageOverlay:
    "absolute inset-0 bg-gradient-to-t from-black/40 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500",
  hotBadge:
    "absolute top-3 left-3 bg-red-500/90 backdrop-blur-md text-white text-[10px] font-bold px-3 py-1 rounded-full flex items-center gap-1 shadow-lg animate-fade-in",
  locationBadge:
    "absolute bottom-3 left-3 flex items-center gap-1.5 bg-black/50 backdrop-blur-md text-white text-[11px] px-2.5 py-1 rounded-lg",
...
export default function FeaturedBikeCard({
  bike,
  className = "",
}: FeaturedBikeCardProps) {
  ...
  const handlePurchase = useCallback(
    (e: React.MouseEvent) => {
      ...
      if (isSeller) {
        addToast(
          "Tài khoản Người bán không thể thực hiện mua xe. Vui lòng đăng nhập bằng tài khoản Người mua.",
          "error",
        );
        return;
      }
  ...
  return (
    <div onClick={handleCardClick} className={`${STYLES.card} ${className}`}>
      {/* Image Container */}
      <div className={STYLES.imageWrapper}>
        ...
        {isHot && (
          <div className={STYLES.hotBadge}>
            <span className="animate-pulse">🔥</span> NỔI BẬT
          </div>
        )}
      ...
      <div className={STYLES.content}>
        ...
        <div className={STYLES.bottomRow}>
          <div className="flex flex-col flex-1 min-w-0">
            ...
            <div className={STYLES.priceValue}>{formatPrice(bike.price)}</div>
          </div>
