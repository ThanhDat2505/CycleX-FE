...
import { 
    Filter, Calendar, Shield, Hash, Search, Info, 
    RefreshCw, ChevronLeft, ChevronRight, Gavel,
    Clock, Tag, User, ExternalLink, AlertCircle, LayoutDashboard, ClipboardList, History
} from "lucide-react";
...
export default function DisputeListClient() {
  ...
  useEffect(() => {
    load();
  }, [status, createdFrom, createdTo, assigneeId, qDebounced, sortBy, sortDir, page, pageSize, isAdmin]);
  ...
  if (loading && !refreshing) {
    return (
        <div className="flex flex-col items-center justify-center py-32 bg-brand-bg min-h-[60vh]">
            ...
            <p className="mt-6 text-[10px] font-black text-gray-500 uppercase tracking-[0.2em] animate-pulse">Syncing cases...</p>
        </div>
    );
  }
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-6 pb-8 border-b border-white/5">
          <div className="animate-slide-up">
            <div className="inline-flex items-center gap-2 bg-white/5 backdrop-blur-md border border-white/10 rounded-full px-4 py-1.5 mb-4">
              ...
              <span className="text-[10px] font-black tracking-[0.2em] uppercase text-gray-400">Dispute Intelligence</span>
            </div>
            <h1 className="text-5xl md:text-6xl font-extrabold tracking-tighter leading-none mb-4">
              Dispute <span className="text-transparent bg-clip-text bg-gradient-to-r from-brand-primary to-orange-400">Queue</span>
            </h1>
            <p className="text-gray-400 text-lg max-w-xl font-medium">
              Quản lý danh sách khiếu nại đang chờ xử lý. Hệ thống tự động phân phối theo mức độ ưu tiên.
            </p>
          </div>
          <button 
            onClick={() => load(true)}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6 mb-10">
          <div className="space-y-2">
            <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest ml-1">Trạng thái</label>
            <div className="relative">
              ...
              >
                {STATUS_OPTIONS.map(opt => <option key={opt.value} value={opt.value} className="bg-brand-bg">{opt.label}</option>)}
              </select>
              <Tag className="absolute right-5 top-1/2 -translate-y-1/2 text-gray-600 pointer-events-none" size={14} />
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            <input 
              type="date" 
              value={createdFrom} 
              onChange={(e) => { setPage(0); setCreatedFrom(e.target.value); }}
          ...
          <div className="space-y-2">
            ...
            <input 
              type="date" 
              value={createdTo} 
              onChange={(e) => { setPage(0); setCreatedTo(e.target.value); }}
              className="w-full px-6 py-4 bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl text-[11px] font-bold text-gray-300 focus:outline-none focus:border-brand-primary/50 [color-scheme:dark]"
            />
          </div>

          <div className="space-y-2 lg:col-span-2">
            <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest ml-1">Tìm kiếm nhanh</label>
            <div className="relative group">
              <input 
                placeholder="Dispute ID / Transaction..."
                value={qInput}
                onChange={(e) => { setPage(0); setQInput(e.target.value); }}
              ...
              />
              <Search className="absolute right-5 top-1/2 -translate-y-1/2 text-gray-600 group-focus-within:text-brand-primary transition-colors" size={18} />
            </div>
          </div>
        </div>
          ...
          <div className="overflow-x-auto min-h-[400px]">
            {items.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-32 text-center">
                ...
                <h3 className="text-2xl font-black text-white mb-2 tracking-tight">Hàng chờ trống</h3>
                <p className="text-gray-500 font-medium">Không có tranh chấp nào khớp với yêu cầu của bạn.</p>
              </div>
            ) : (
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="border-b border-white/5 bg-white/[0.01]">
                    <th className="px-8 py-6 text-[10px] font-black text-gray-500 uppercase tracking-[0.2em]">ID</th>
                    <th className="px-8 py-6 text-[10px] font-black text-gray-500 uppercase tracking-[0.2em]">Transaction & Listing</th>
                    <th className="px-8 py-6 text-[10px] font-black text-gray-500 uppercase tracking-[0.2em] text-center">Trạng thái</th>
                    <th className="px-8 py-6 text-[10px] font-black text-gray-500 uppercase tracking-[0.2em]">Khởi tạo</th>
                    <th className="px-8 py-6 text-[10px] font-black text-gray-500 uppercase tracking-[0.2em]">Assignee</th>
                ...
                <tbody className="divide-y divide-white/5">
                  {items.map((row) => (
                    <tr key={row.id} className="group hover:bg-white/[0.02] transition-colors">
                      ...
                      <td className="px-8 py-6">
                        <div>
                          <p className="text-sm font-black text-white mb-1 group-hover:text-brand-primary transition-colors">{row.listingTitle}</p>
                          <div className="flex items-center gap-2">
                             <span className="text-[10px] font-black text-gray-600 bg-white/5 px-2 py-0.5 rounded border border-white/5">TX-{row.transactionId}</span>
                             <span className="text-[10px] font-bold text-gray-500 truncate max-w-[200px]">{row.reason}</span>
                          </div>
                        </div>
                      </td>
                      <td className="px-8 py-6 text-center">
                        <span className={`inline-flex px-4 py-1.5 rounded-xl text-[10px] font-black tracking-widest uppercase border transition-all ${statusStyle(row.status)}`}>
                          {row.status}
                        </span>
                      </td>
                      <td className="px-8 py-6">
                        <div className="flex flex-col">
                          <span className="text-[11px] font-black text-white">{row.createdAt.split('T')[0]}</span>
                          <span className="text-[9px] font-bold text-gray-600 uppercase tracking-tighter mt-1">{row.createdAt.split('T')[1]?.split('.')[0] || "Unknown"}</span>
                        </div>
                      </td>
                      <td className="px-8 py-6">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs font-bold text-gray-400">{row.assigneeName || "Chưa phân công"}</span>
                        </div>
                      </td>
          ...
          {!loading && totalPages > 1 && (
            <div className="px-8 py-6 border-t border-white/5 flex items-center justify-between bg-white/[0.01]">
              <div className="flex items-center gap-4">
                ...
                <p className="text-[10px] text-gray-500 font-black uppercase tracking-widest">
                  Hiển thị <span className="text-white">{items.length}</span> / {totalItems} tranh chấp
                </p>
              </div>
              <div className="flex gap-4">
                ...
                <div className="flex items-center gap-2">
                   ...
                   >
                     <option value={10} className="bg-brand-bg">10 cases</option>
                     <option value={20} className="bg-brand-bg">20 cases</option>
                     <option value={50} className="bg-brand-bg">50 cases</option>
                   </select>
                </div>
                <button 
