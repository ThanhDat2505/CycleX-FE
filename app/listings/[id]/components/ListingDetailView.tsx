...
const STYLES = {
  ...
  breadcrumbLink:
    "hover:text-brand-primary transition-colors flex items-center gap-1",
  ...
  badgeUsed:
    "px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-wider bg-amber-100 text-amber-700 border border-amber-200",
  badgeType:
    "px-4 py-1.5 bg-blue-50 text-blue-700 border border-blue-100 rounded-full text-xs font-bold uppercase tracking-wider",
  ...
  specCard:
    "bg-gray-50/50 border border-gray-100 p-4 rounded-2xl flex flex-col items-center text-center gap-2 hover:bg-white hover:shadow-sm transition-all duration-300 group",
  specIcon:
    "p-2 bg-white rounded-xl text-brand-primary shadow-sm group-hover:scale-110 transition-transform",
  ...
  ctaButton:
    "block w-full px-8 py-5 bg-brand-primary text-white text-center font-bold rounded-2xl hover:bg-brand-primary-hover transition-all shadow-xl shadow-brand-primary/20 active:scale-[0.98]",
  ctaHint: "text-sm text-gray-400 text-center mt-3 font-medium",
  // Mobile Sticky CTA
  mobileStickyCta:
    "lg:hidden fixed bottom-0 left-0 right-0 p-4 bg-white/80 backdrop-blur-md border-t border-gray-100 z-40 shadow-[0_-10px_20px_rgba(0,0,0,0.05)]",
  mobileCtaButton:
    "w-full py-4 bg-brand-primary text-white text-center font-bold rounded-xl active:scale-[0.98] transition-transform shadow-lg shadow-brand-primary/20",
  // Description
  descSection: "border-t border-gray-100 pt-10",
  descTitle: "text-2xl font-bold text-gray-900 mb-6",
  descContent:
    "prose prose-blue max-w-none text-gray-600 leading-relaxed bg-gray-50/30 p-8 rounded-3xl border border-gray-100",
  ...
  inspectionTitle:
    "text-2xl font-bold text-gray-900 mb-6 flex items-center gap-2",
  inspectionCard:
    "relative overflow-hidden bg-gradient-to-br from-green-50 to-white border-2 border-green-100 rounded-3xl p-8 shadow-sm",
  inspectionStamp:
    "absolute -right-6 -top-6 text-green-100 opacity-50 rotate-12",
  inspectionBadge:
    "inline-flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-bold bg-green-600 text-white shadow-lg shadow-green-600/20 mb-6",
  inspectionGrid: "grid grid-cols-1 md:grid-cols-2 gap-8",
  inspectionItem: "flex items-start gap-4",
  inspectionItemIcon:
    "p-3 bg-white rounded-2xl text-green-600 shadow-sm border border-green-50",
  inspectionItemLabel:
    "text-xs font-bold text-gray-400 uppercase tracking-widest mb-1",
  inspectionItemValue: "text-gray-900 font-bold",
  inspectionBadgeFailed:
    "inline-flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-bold bg-red-600 text-white shadow-lg shadow-red-600/20 mb-6",
  inspectionBadgeOther:
    "inline-flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-bold bg-amber-500 text-white shadow-lg shadow-amber-500/20 mb-6",
  inspectionNoteValue: "text-gray-900 flex-1 font-medium leading-relaxed",
  // Seller Coming Soon
  sellerSection: "border-t border-gray-100 pt-10 pb-20",
  sellerCard:
    "bg-brand-bg rounded-3xl p-8 text-center border-2 border-brand-primary/5",
...
function getInspectionStatusStyles(status: string) {
  if (status === "PASSED")
    return { badge: STYLES.inspectionBadge, icon: <CheckCircle2 size={16} /> };
  if (status === "FAILED")
    return {
      badge: STYLES.inspectionBadgeFailed,
      icon: <CheckCircle2 size={16} />,
    };
  return {
    badge: STYLES.inspectionBadgeOther,
    icon: <CheckCircle2 size={16} />,
  };
}
...
export default function ListingDetailView({
  ...
  const inspectionStyles = getInspectionStatusStyles(
    listing.inspectionStatus || "",
  );
  ...
  return (
    <div className={STYLES.wrapper}>
      ...
      <div className={STYLES.grid}>
        ...
        {/* Right: Basic Info */}
        <div
          className={`${STYLES.infoSection} animate-slide-up`}
        ...
        >
          ...
          {/* Purchase Button - Desktop (NOT for SELLER, only AVAILABLE products) */}
          {!isSeller && listing.productStatus === "AVAILABLE" && (
            <div className={STYLES.ctaSection}>
              <Link href={purchaseHref} className={STYLES.ctaButton}>
      ...
      {/* Sticky Mobile CTA (NOT for SELLER, only AVAILABLE products) */}
      {!isSeller && listing.productStatus === "AVAILABLE" && (
        <div className={STYLES.mobileStickyCta}>
          <Link href={purchaseHref} className={STYLES.mobileCtaButton}>
      ...
      {hasInspection && (
        <div
          className={`${STYLES.inspectionSection} animate-slide-up`}
            ...
            <div className={STYLES.inspectionGrid}>
              {listing.inspectionDate && (
                <div className={STYLES.inspectionItem}>
                  <div className={STYLES.inspectionItemIcon}>
                    <Calendar size={24} />
                  </div>
                  <div>
                    <div className={STYLES.inspectionItemLabel}>
                      {MESSAGES.DETAIL_INSPECTION_DATE}
                    </div>
                    <div className={STYLES.inspectionItemValue}>
                      {new Date(listing.inspectionDate).toLocaleDateString(
                        "vi-VN",
                        {
                          year: "numeric",
                          month: "long",
              ...
              {listing.inspectionNotes && (
                <div className={STYLES.inspectionItem}>
                  ...
                  <div className="flex-1">
                    ...
                    <div className={STYLES.inspectionNoteValue}>
                      {listing.inspectionNotes}
                    </div>
                  </div>
                </div>
      ...
      <div
        className={`${STYLES.sellerSection} animate-slide-up`}
      ...
      >
        <div className={STYLES.sellerCard}>
          ...
          <p className={STYLES.sellerDesc}>
            {MESSAGES.DETAIL_SELLER_INFO_DESC}
          </p>
        </div>
      </div>
    </div>
