...
const STEPS = [
  {
    id: "created",
    label: "Đã đặt hàng",
    status: TRANSACTION_STATUS.PENDING_SELLER_CONFIRM,
  },
  {
    id: "pending",
    label: "Chờ xác nhận",
  ...
  },
  {
    ...
    status: TRANSACTION_STATUS.IN_DELIVERY,
  },
  {
    id: "completed",
    label: "Hoàn thành",
...
export default function OrderTimeline({
  ...
  if (status === TRANSACTION_STATUS.DISPUTED) {
    return (
      <div className="w-full bg-orange-50 border border-orange-100 rounded-xl p-6 text-center">
        <div className="flex flex-col items-center gap-2">
          <div className="w-12 h-12 bg-orange-100 rounded-full flex items-center justify-center text-orange-600 mb-2">
            ...
            >
              <path
                ...
                strokeLinejoin="round"
                strokeWidth={2}
                d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L3.034 16.5c-.77.833.192 2.5 1.732 2.5z"
              />
            </svg>
          </div>
  ...
  switch (status) {
    ...
    case TRANSACTION_STATUS.PENDING_DELIVERY:
      currentStepIndex = 2;
      break;
    case TRANSACTION_STATUS.IN_DELIVERY:
      currentStepIndex = 3;
      break;
          ...
          return (
            ...
            >
              ...
              <div className="absolute top-12 left-1/2 transform -translate-x-1/2 w-32 text-center">
                <p
                  className={`text-xs md:text-sm font-bold transition-colors ${isCompleted ? "text-gray-900" : "text-gray-400"}`}
                >
                  {step.label}
                </p>
