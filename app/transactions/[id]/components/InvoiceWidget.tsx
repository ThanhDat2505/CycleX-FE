...
export default function InvoiceWidget({
  ...
  const typeLabel =
    TRANSACTION_TYPE_LABELS[
      transaction.transactionType as keyof typeof TRANSACTION_TYPE_LABELS
    ...
    <div className="bg-white rounded-2xl shadow-card border border-gray-100 overflow-hidden sticky top-24 animate-slide-in-right">
      ...
      <div className="p-6 space-y-4">
        ...
        <div className="flex justify-between items-center pb-4 border-b border-gray-100">
          ...
          <span
            className={`font-bold px-3 py-1 rounded-full text-xs ${isPurchase ? "bg-orange-50 text-brand-primary" : "bg-orange-100 text-orange-800"}`}
          >
        ...
        <div className="space-y-2 text-sm">
          {(transaction.listingPrice ?? 0) > 0 && (
            <div className="flex justify-between">
              ...
              <span className="font-medium text-gray-900">
                {formatPrice(transaction.listingPrice ?? 0)}
              </span>
            </div>
          )}
          {!isPurchase && (transaction.depositAmount ?? 0) > 0 && (
            <div className="flex justify-between">
              <span className="text-gray-500">Tiền cọc (10%)</span>
              <span className="font-medium text-gray-900">
                {formatPrice(transaction.depositAmount ?? 0)}
              </span>
            </div>
          )}
          <div className="flex justify-between">
          ...
          <div className="flex justify-between">
            ...
            <span className="font-medium text-gray-900">
              {formatPrice(transaction.inspectionFee)}
            </span>
          </div>
        </div>
        ...
        <div className="pt-4 border-t border-gray-100">
          <div className="flex justify-between items-end">
            ...
            <span className="text-2xl font-extrabold text-brand-primary leading-none">
              {formatPrice(
                (isPurchase
                  ? (transaction.listingPrice ?? 0)
                  : (transaction.depositAmount ?? 0)) +
                  transaction.platformFee +
                  transaction.inspectionFee,
              )}
            </span>
          </div>
            ...
            {viewerRole === "SELLER" ? (
              <>
                ...
                <p className="text-xs text-gray-400 text-center mt-3 leading-relaxed">
                  Giao dịch sẽ được chuyển sang trạng thái &quot;Đã xác
                  nhận&quot;. Bạn không thể hoàn tác hành động này.
                </p>
              </>
            ) : (
              <>
        ...
        {isConfirmed && (
          <div className="pt-4 mt-2 bg-green-50 rounded-lg p-4 border border-green-100 text-center">
            <div className="text-green-700 font-bold flex items-center justify-center gap-2 mb-1">
              <svg
                className="w-5 h-5 bg-green-200 rounded-full p-0.5 text-green-700"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
