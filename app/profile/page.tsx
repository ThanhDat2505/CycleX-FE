...
export default function ProfilePage() {
    ...
    const handleUpdateProfile = async (data: UpdateProfileRequest) => {
        ...
        try {
            ...
            const updatedProfile = await userService.updateUserProfile(profile.userId, data);
    ...
    const handleChangePassword = async (data: ChangePasswordRequest) => {
        ...
        try {
            ...
            const errorMsg = (error as Error)?.message || MESSAGES.S04_ERROR_CHANGE_PASSWORD;
        ...
        <main className="min-h-screen bg-gray-50 pt-24 pb-12">
            <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <nav className="flex items-center text-sm text-gray-500 mb-6">
                    ...
                    <span className="text-gray-900 font-medium">{MESSAGES.S04_PAGE_TITLE}</span>
                </nav>
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                    <div className="flex flex-col md:flex-row min-h-[600px]">
                        ...
                        <div className="w-full md:w-64 border-b md:border-b-0 md:border-r border-gray-100 bg-gray-50/50 p-6">
                            <h2 className="text-xl font-bold tracking-tight text-gray-900 mb-6">Cài đặt</h2>
                            <nav className="space-y-2">
                                <button
                                    onClick={() => setActiveTab('info')}
                                    className={`w-full flex items-center px-4 py-3 text-sm font-medium rounded-xl transition-all ${activeTab === 'info'
                                        ? 'bg-blue-50 text-blue-700 shadow-sm ring-1 ring-blue-700/10'
                                        : 'text-gray-600 hover:bg-white hover:text-gray-900 hover:shadow-sm'
                                        }`}
                                >
                                    <User className={`w-5 h-5 mr-3 ${activeTab === 'info' ? 'text-blue-600' : 'text-gray-400'}`} />
                                ...
                                >
                                    <Lock className={`w-5 h-5 mr-3 ${activeTab === 'security' ? 'text-blue-600' : 'text-gray-400'}`} />
                                ...
                                <div className="space-y-6 max-w-2xl">
                                    <div className="border-b border-gray-200 pb-5">
                                        ...
                                        <p className="mt-2 text-sm text-gray-500">
                                            Quản lý thông tin cá nhân và cách bạn hiển thị với người khác trên CycleX.
                                        </p>
                                    </div>
                                    <ProfileInfoForm
                                        profile={profile}
