...
export async function getDeliverySummary(
  ...
  try {
    const data = await apiCallGET<any>(
      ENDPOINTS.shipper.dashboardSummaryLegacy,
    );
...
const MOCK_DELIVERIES: Delivery[] = [
  {
    ...
    bike: {
      name: "Giant Escape 3",
      image:
        "https://images.unsplash.com/photo-1485965120184-e220f721d03e?auto=format&fit=crop&q=80&w=300",
    },
    status: "ASSIGNED",
  ...
  },
  {
    ...
    bike: {
      name: "Asama TRK",
      image:
        "https://images.unsplash.com/photo-1576435728678-35d01fda69e6?auto=format&fit=crop&q=80&w=300",
    },
    status: "FAILED",
  ...
  },
  {
    ...
    bike: {
      name: "Trinx M136",
      image:
        "https://images.unsplash.com/photo-1507035895480-27fb4af89056?auto=format&fit=crop&q=80&w=300",
    },
    status: "DELIVERED",
...
export async function getAssignedDeliveries(
  ...
  try {
    ...
    const queryParams =
      filter !== "ALL"
        ? `?status=${filter}&page=0&pageSize=20`
        : "?page=0&pageSize=20";
    const dataResponse = await apiCallGET<any>(
      `${ENDPOINTS.shipper.assignedDeliveries}${queryParams}`,
    );
...
export async function getDeliveryDetail(
  ...
  try {
    const data = await apiCallGET<any>(
      ENDPOINTS.shipper.deliveryDetail(deliveryId),
    );
    if (!data) return null;
