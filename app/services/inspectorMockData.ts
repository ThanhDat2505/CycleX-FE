...
export const mockInspectorListings: MockListing[] = [
  {
    ...
    waitingDays: 2,
    images: {
      main: "https://images.unsplash.com/photo-1485965120184-e220f721d03e",
      thumbs: [],
    },
    history: [
      {
  ...
  },
  {
    ...
    history: [
      ...
      },
      {
        ...
        tag: "REVIEWING",
        desc: "Inspector started reviewing",
      },
    ],
  },
  {
  ...
  },
  {
    ...
    waitingDays: 3,
    images: {
      main: "https://images.unsplash.com/photo-1541625602330-2277a4c46182",
      thumbs: [],
    },
    history: [
      {
  ...
  },
  {
    ...
    waitingDays: 4,
    images: {
      main: "https://images.unsplash.com/photo-1528629297340-d1d466945dc5",
      thumbs: [],
    },
    history: [
      {
...
export async function handleInspectorMockRequest(
  path: string,
  init?: RequestInit,
): Promise<any> {
  ...
  if (path.includes("/dashboard/stats")) {
    return {
      pendingCount: mockInspectorListings.filter((l) => l.status === "PENDING")
        .length,
      reviewingCount: mockInspectorListings.filter(
        (l) => l.status === "REVIEWING",
      ).length,
      approvedCount: mockInspectorListings.filter(
        (l) => l.status === "APPROVED",
      ).length,
      rejectedCount: mockInspectorListings.filter(
        (l) => l.status === "REJECTED",
      ).length,
      disputeCount: mockInspectorListings.filter((l) => l.status === "DISPUTE")
        .length,
    };
  }
  ...
  if (path.includes("/reject") && method === "POST") {
    ...
    if (listingId) {
      const idx = mockInspectorListings.findIndex(
        (l) => l.id === String(listingId),
      );
      if (idx !== -1) {
