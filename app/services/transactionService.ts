...
export async function createPurchaseRequest(
  ...
  const dataResponse = await apiCallPOST<Transaction>(
    ENDPOINTS.transactions.purchaseRequest(data.listingId),
    payload,
  );
...
export async function getTransactionDetail(
    ...
    const mockTransaction: import("../types/transaction").TransactionWithDetails =
      {
        ...
        listingImage:
          "https://images.unsplash.com/photo-1576435728678-be95d398b646?auto=format&fit=crop&q=80&w=500",
  ...
  const dataResponse = await apiCallGET<
    import("../types/transaction").TransactionWithDetails
  >(ENDPOINTS.transactions.detail(transactionId, role));

  // Validate backend response
  if (!dataResponse || typeof dataResponse !== "object") {
    throw new Error(
      "Invalid backend response: Expected a transaction detail object",
    );
  }
  ...
  validateEnum<import("../types/transaction").TransactionType>(
    dataResponse.transactionType,
    ["PURCHASE", "DEPOSIT"],
    `${ctx}.transactionType`,
  );
...
export async function getBuyerTransactions(
  ...
  const dataResponse = await apiCallGET<
    import("../types/transaction").TransactionWithDetails[]
  >(ENDPOINTS.transactions.buyerById(buyerId));
...
export async function acceptTransaction(
  ...
  await apiCallPOST(ENDPOINTS.transactions.sellerConfirm(transactionId), {
    note: "Đã xác nhận từ phía người bán",
  });
  return true;
}
...
export async function cancelTransaction(
  ...
  await apiCallPOST(ENDPOINTS.transactions.buyerCancel(transactionId), {});
  return true;
}
