...
export async function getFeaturedBikes(): Promise<HomeBike[]> {
  ...
  const bikes = await apiCallGET<HomeBike[]>(ENDPOINTS.listings.featured);
...
export async function getAllListings(page: number = 1): Promise<HomeBike[]> {
  ...
  const bikes = await apiCallGET<HomeBike[]>(ENDPOINTS.listings.all);
...
export async function searchListings(
  ...
  if (USE_MOCK_API) {
    ...
    if (keyword && keyword.trim().length > 0) {
      ...
      results = results.filter(
        (listing) => listing.title.toLowerCase().includes(keywordLower),
    ...
    if (filters?.brands && filters.brands.length > 0) {
      results = results.filter(
        (listing) => listing.brand && filters.brands!.includes(listing.brand), // Add null check
      );
    }
  ...
  try {
    ...
    if (filters?.bikeTypes)
      filters.bikeTypes.forEach((t) => params.append("bikeType", t));
    if (filters?.brands)
    ...
    }>(`${ENDPOINTS.listings.all}?${params.toString()}`);
...
export async function searchSellerListings(
  ...
  const data = await apiCallPOST<{
    items: HomeBike[];
    pagination: PaginationInfo;
  }>(ENDPOINTS.listings.sellerSearchLegacy, requestBody);
...
export async function getListingDetail(
  ...
  try {
    const data = await apiCallGET<ListingDetail>(
      ENDPOINTS.listings.detail(listingId),
    );
...
export async function getSellerListingDetail(
  ...
  const data = await apiCallPOST<ListingDetail>(
    ENDPOINTS.listings.sellerDetailLegacy,
    { sellerId, listingId },
  );
