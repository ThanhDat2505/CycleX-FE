...
function normalizeListingResponse(
  response: ListingApiResponse,
  context: string,
): Listing {
  validateResponse(response, context);

  const normalizedId =
    typeof response.id === "number" ? response.id : response.listingId;

  if (typeof normalizedId !== "number") {
    throw new Error(
      `Invalid response: ${context}.id (or listingId) must be a number`,
    );
  }
...
function mapStatusFromBackend(status: string): ListingStatus {
  ...
  if (
    ...
    normalized === "WAITING_INSPECTOR_REVIEW" ||
    normalized === "ARCHIVED"
  ) {
    return "PENDING";
  }
...
function mapSearchItemToListing(
  item: SellerListingSearchItemResponse,
): Listing {
...
async function attachMainImageToListings(
  sellerId: number,
  listings: Listing[],
): Promise<Listing[]> {
  ...
  const listingsWithImage = await Promise.all(
    listings.map(async (listing) => {
      try {
        ...
        return mainImageUrl
          ? ({ ...listing, mainImageUrl } as Listing)
          : listing;
      } catch {
...
export async function getMyListings(
  ...
  const response = await apiCallGET<SellerListingSearchPageResponse>(
    `${ENDPOINTS.listings.sellerSearch(sellerId)}?${queryStr}`,
  );
  ...
  const listingsWithImage = await attachMainImageToListings(
    sellerId,
    mappedListings,
  );
...
export async function createListing(
  ...
  const response = await apiCallPOST<ListingApiResponse>(
    ENDPOINTS.listings.sellerCreate(sellerId),
    { ...body, sellerId, saveDraft: false },
  );
  const normalizedResponse = normalizeListingResponse(
    response,
...
export async function updateDraft(
  ...
  if (!sellerId || sellerId <= 0) {
    throw new Error("Seller ID is required to update draft listing.");
  }

  const response = await apiCallPATCH<ListingApiResponse>(
    ENDPOINTS.listings.sellerListingDetail(sellerId, listingId),
    { sellerId, ...rest },
  );
  const normalizedResponse = normalizeListingResponse(
    response,
...
export async function submitDraft(
  ...
  if (!sellerId || sellerId <= 0) {
    throw new Error("Seller ID is required to submit draft listing.");
  }

  const response = await apiCallPOST<ListingApiResponse>(
    ENDPOINTS.listings.sellerDraftSubmit(sellerId, listingId),
    {},
  );
  const normalizedResponse = normalizeListingResponse(
    response,
...
export async function previewListing(
  ...
  const response = await apiCallGET<ListingApiResponse>(
    ENDPOINTS.listings.sellerListingPreview(sellerId, listingId),
  );
  const normalizedResponse = normalizeListingResponse(
    response,
    "previewListing response",
  );
...
export async function submitListing(
  ...
  // Keep this API for backward compatibility with legacy callers.
  return await submitDraft(listingId, sellerId);
}
...
export async function getDrafts(
  ...
  const response = await apiCallGET<SellerListingSearchPageResponse>(
    `${ENDPOINTS.listings.sellerDrafts(sellerId)}?${queryParams}`,
  );
...
export async function getDashboardStats(sellerId: number): Promise<any> {
  ...
  return await apiCallGET(ENDPOINTS.listings.sellerDashboardStats(sellerId));
}
...
export async function getListingDetail(
  sellerId: number,
  listingId: number,
): Promise<SellerListingDetailResponse> {
  const response = await apiCallGET<SellerListingDetailResponse>(
    ENDPOINTS.listings.sellerListingPreview(sellerId, listingId),
  );
...
export async function getRejectionReason(
  sellerId: number,
  listingId: number,
): Promise<RejectionInfo> {
  if (USE_MOCK_API) {
    await new Promise((resolve) => setTimeout(resolve, API_DELAY_MS));
    return {
      listingId,
      reasonCode: "INVALID_INFO",
      reasonText: "Mock rejection reason",
      note: "Please update your listing",
    };
  }

  return await apiCallGET<RejectionInfo>(
    ENDPOINTS.listings.sellerListingRejection(sellerId, listingId),
  );
}
...
export async function deleteDraft(
  ...
  await apiCallDELETE(
    ENDPOINTS.listings.sellerDrafts(sellerId) + `/${draftId}`,
  );
}
...
export async function getListingImages(
  ...
  return await apiCallGET<ListingImage[]>(
    ENDPOINTS.listings.sellerListingImages(sellerId, listingId),
  );
}
...
export async function uploadListingImage(
  ...
  if (USE_MOCK_API) {
    ...
    return {
      id: Math.floor(Math.random() * 10000),
      imagePath,
      imageOrder: _imageOrder ?? 1,
    };
  }

  return await apiCallPOST<ListingImage>(
    ENDPOINTS.listings.sellerListingImages(sellerId, listingId),
    {
      imagePath,
...
export async function deleteListingImage(
  ...
  await apiCallDELETE(
    ENDPOINTS.listings.sellerListingImageDetail(sellerId, listingId, imageId),
  );
}
