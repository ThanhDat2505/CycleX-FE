...
export const authService = {
  ...
  login: async (
    ...
    try {
      ...
      const data = await apiCallPOST<LoginResponse>("/auth/login", {
        email,
        password,
        rememberMe,
      } as LoginRequest);
  ...
  verifyOtp: async (email: string, otp: string): Promise<VerifyOtpResponse> => {
    try {
      const data = await apiCallPOST<VerifyOtpResponse>("/auth/verify-otp", {
        email,
        otp,
      } as VerifyOtpRequest);
  ...
  sendOtp: async (email: string): Promise<SendOtpResponse> => {
    try {
      const data = await apiCallPOST<SendOtpResponse>("/auth/send-otp", {
        email,
      } as SendOtpRequest);
  ...
  forgotPassword: async (email: string): Promise<{ message: string }> => {
    try {
      const data = await apiCallPOST<{ message: string }>(
      ...
      throw new Error(
        error.response?.data?.message ||
          "Có lỗi xảy ra khi yêu cầu cấp lại mật khẩu.",
      );
    }
  },
