...
export const authService = {
  ...
  login: async (
    ...
    try {
      ...
      const data = await apiCallPOST<LoginResponse>(ENDPOINTS.auth.login, {
        email,
        password,
        rememberMe,
      } as LoginRequest);
  ...
  register: async (
    ...
    try {
      const data = await apiCallPOST<RegisterResponse>(
  ...
  verifyOtp: async (email: string, otp: string): Promise<VerifyOtpResponse> => {
    ...
    try {
      const data = await apiCallPOST<VerifyOtpResponse>(
  ...
  sendOtp: async (email: string): Promise<SendOtpResponse> => {
    ...
    try {
      const data = await apiCallPOST<SendOtpResponse>(ENDPOINTS.auth.sendOtp, {
        email,
      } as SendOtpRequest);
