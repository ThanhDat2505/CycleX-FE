...
import { apiCallPOST, apiCallPUT, apiCallDELETE } from '../utils/apiHelpers';
 ...
 * Endpoint: DELETE /api/seller/{sellerId}/drafts/{listingId}
 * 
 * @param sellerId - Seller ID (path variable)
 * @param listingId - Draft listing ID to delete (path variable)
 * @returns Promise<void>
 */
export async function deleteDraft(sellerId: number, listingId: number): Promise<void> {
    if (USE_MOCK_API) {
        await new Promise(resolve => setTimeout(resolve, API_DELAY_MS));

        // Find and remove from mock listings
        const listingIndex = mockListings.findIndex(l => l.id === listingId);
        if (listingIndex === -1) {
            throw new Error(`Draft listing ${listingId} not found`);
        }
    ...
    await apiCallDELETE(`/seller/${sellerId}/drafts/${listingId}`);
}
